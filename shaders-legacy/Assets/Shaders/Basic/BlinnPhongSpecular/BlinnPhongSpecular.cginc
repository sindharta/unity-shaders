...
#ifndef SIN_BASIC_BLINN_PHONG_SPECULAR
#define SIN_BASIC_BLINN_PHONG_SPECULAR
...
PS_IN BlinnPhongSpecularVS(appdata_base v) {
	PS_IN o;
...
float4 BlinnPhongSpecularPS(PS_IN input) : COLOR {
    ...
    const float n_dot_half = dot(n,half_vector);
    const float3 spec_term = pow(saturate(n_dot_half),_SpecularPower);
